...
async function loadEditorJSButton() {
  const { default: ButtonEditor } = await import("./ButtonEditor");
  return ButtonEditor;
}
...
async function loadEditorJSYoutubeEmbed() {
  const { default: YoutubeEmbed } = await import("editorjs-youtube-embed");
  return YoutubeEmbed;
}
...
export default function Editor({
  ...
  React.useEffect(() => {
    const initEditors = async () => {
      ...
      const Button = await loadEditorJSButton();
      const Quote = await loadEditorJSQuote();
      const RawTool = await loadEditorJSRaw();
      const YoutubeEmbed = await loadEditorJSYoutubeEmbed();
      rows.forEach((row) => {
                ...
                {row.columns.map((column) => {
                  return (
                    <div
                      ...
                      key={column.id + "asd"}
                    >
                      <div id={column.id} className="columna" />
                    </div>
                  );
                })}
              </div>
