        ...
        switch (block.type) {
          ...
          case "youtubeEmbed":
            try {
              return (
                <YouTube
                  videoId={new URL(block.data.url).searchParams.get("v")}
                  id="video"
                  iframeClassName="w-full mb-5"
                  key={index}
                />
              );
            } catch {
          ...
          case "button":
            return (
              <div>
                <div className="text-[#63687A] monumental px-5 py-2 h-fit w-full inline-block">
                  <img src="/images/vector3.svg" className="inline-block"></img>
                  <p className="inline-block ml-3 w-full">{block.data.text}</p>
                  <div
                    key={index}
                    onClick={() => {
                      window.open(block.data.url);
                    }}
                    className="text-white float-right mt-3 bg-[#1f1f1f] monumental px-5 py-2 rounded-[12px] h-fit w-fit inline-block cursor-pointer"
                  >
