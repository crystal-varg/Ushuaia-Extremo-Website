...
module.exports = async function (request, response) {
  const { nombre, tel, email, message, date, adults, children, babies, url } = request.body;
  ...
  await sendEmail({
    to: getSetting("storeEmail", "joelroaroldan@gmail.com"),
    subject: `Nueva consulta desde ${url}`,
    html: template,
  });

  return response.json({ message: "Correo de consulta enviado correctamente" });
};
