...
function AddToCart({ stockAvaibility, loading = false, error, name, variant }) {
  return (
      ...
      <div className="mt-4">
        ...
        {stockAvaibility === false && (
          <Button
            title={_("CONSULTAR")}
            onAction={() => {
              const mensaje = `Hola! Me interesa consultar por el producto: ${name}`;
              const url = `https://wa.me/5492901640089?text=${encodeURIComponent(
                mensaje
              )}`;
...
export default function ProductForm({ product, action, variant }) {
